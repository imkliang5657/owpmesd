...
foreach ($data['vessel_requirements'] as $key => $value) {
    $vessel_requirements .= <<<HTML
      <div class="row mb-2">
...
$vessel_requirements .= "</div></div>";
?>

<?php
if (!empty($data['foreign_vessel']['vessel_detail_id'])) {
    $category = $data['foreign_vessel']['vessel_category_name'];
    $detail = <<< HTML
    <div class="card">
      <div class="card-header">
        $category 規格
  ...
  HTML;
    ...
    $startPrinting = false;
    foreach ($data['foreign_vessel'] as $key => $value) {
...
<div class="container mt-5">
  ...
  <button class="btn btn-warning" type="button" data-bs-toggle="collapse" data-bs-target="#collapseExample3" aria-expanded="false" aria-controls="collapseExample">
    查看船舶文件
  </button>
    ...
    <div class="row my-4">
      <div class="col-md-12">
        <div class="card">
          ...
          <div class="card-body">
            <a href="<?= $data['files']['certificate_of_classification']['name']?>" class="btn btn-sm btn-outline-dark m-1" role="button" target="_blank">
              船級證書 <i class="bi bi-file-earmark-arrow-down"></i>
            </a>
            <a href="<?= $data['files']['specification']['name']?>" class="btn btn-sm btn-outline-dark m-1" role="button" target="_blank">
              船級規格 <i class="bi bi-file-earmark-arrow-down"></i>
            </a>
            <a href="<?= $data['files']['reference']['name'] ?? ''?>" class="btn btn-sm btn-outline-dark m-1" role="button" target="_blank" style="display: <?= isset($data['files']['reference']['name']) ? 'display' : 'none' ?>">
              其他設備或佐證資料 <i class="bi bi-file-earmark-arrow-down"></i>
            </a>
          </div>
        </div>
      </div>
  ...
  <div class="row my-4">
    <form action="?url=applications/application/review&id=<?= $data['id'] ?>" method="post">
      <div class="col-md-12">
        <div class="card <?= $stage_background_color[0] ?>">
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-3" style="text-align: end">
                <button type="submit" name="status" value="rejected" class="btn btn-danger" <?= $stage_disabled[0] ? 'disabled' : '' ?> >
                  退件 <i class="bi bi-ban-fill"></i>
                </button>
                <button type="submit" name="status" value="prepare_conference" class="btn btn-success" <?= $stage_disabled[0] ? 'disabled' : '' ?> >
          ...
          <div class="card-body">
            <div class="row p-1">
              <div class="col-5">
                <div class="my-2">
                  <label for="formGroupExampleInput" class="form-label">意見欄</label>
                  <input name="command" type="text" class="form-control" id="formGroupExampleInput"
                         value="<?= $data['command'] ?? '' ?>" placeholder="如要退件，煩請在該欄填寫理由，方便申請者修改！" <?= $stage_disabled[0] ? 'disabled' : '' ?>>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-4" style="text-align: end">
                <a href="?url=applications/application/return&id=<?= $data['id'] ?>&status=submitted" class="btn btn-danger <?= $stage_disabled[1] ? 'disabled' : '' ?> ">
                  退回上階段 <i class="bi bi-arrow-up-square-fill"></i>
                </a>
        ...
        <div class="card <?= $stage_background_color[2] ?>">
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-3" style="text-align: end">
                <a href="?url=applications/application/return&id=<?= $data['id'] ?>&status=prepare_conference" class="btn btn-danger" <?= $stage_disabled[2] ? 'disabled' : '' ?> >
                  退回上階段 <i class="bi bi-arrow-up-square-fill"></i>
                </a>
