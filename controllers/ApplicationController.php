...
class ApplicationController extends Controller
{
    ...
    public function retrieveByAdmin(): void
    {
        ...
        // application, application_information
        $data = $this->applicationModel->retrieve($getData['id']);

        // vessel_requirement
        $data['vessel_requirements'] = $this->convertSpecifications($data['vessel_category_id'], $this->applicationVesselRequirementModel->retrieveByApplicationId($getData['id']));

        // foreign_vessel
        $vesselData = $this->applicationForeignVesselModel->retrieveByApplicationId($getData['id']);
    ...
    public function review()
    {
        $getData = $this->retrieveGetData();
        $postData = $this->retrievePostData();
        $this->applicationModel->updateStatusAndCommandById($getData['id'], $postData['status'], $postData['command']);
    ...
    {
        $getData = $this->retrieveGetData();
        $this->applicationModel->updateStatusById($getData['id'], $getData['status']);
        $this->redirect('?url=page/applications/application&id='.$getData['id']);
    }
    ...
    public function heldConference()
    {
        ...
        if ($this->applicationFileModel->retrieveByApplicationIdAndType($getData['id'], 'application_letter')) {
            $this->applicationFileModel->update($getData['id'], $targetFilePath, 'application_letter');
        } else {
        ...
        if ($this->applicationFileModel->retrieveByApplicationIdAndType($getData['id'], 'conference_notice')) {
            $this->applicationFileModel->update($getData['id'], $targetFilePath, 'conference_notice');
        } else {
    ...
    public function close()
    {
        ...
        if ($this->applicationFileModel->retrieveByApplicationIdAndType($getData['id'], 'conference_record')) {
            $this->applicationFileModel->update($getData['id'], $targetFilePath, 'conference_record');
        } else {
        ...
        if ($this->applicationFileModel->retrieveByApplicationIdAndType($getData['id'], 'consent_letter')) {
            $this->applicationFileModel->update($getData['id'], $targetFilePath, 'consent_letter');
        } else {
