...
class Application
{
    ...
    public function retrieveByStatus($status): bool|array
    {
        $query = "SELECT 
                    ...
                    `application_foreign_vessel`.`name` as `vessel`
    ...
    public function updateStatusAndCommandById($id, $status, $command): void
    {
        $query = "UPDATE `applications` SET `status` = :status, `command` = :command WHERE `applications`.`id` = :id";
        $this->db->query($query);
        $this->db->bind('id', $id);
        $this->db->bind('status', $status);
