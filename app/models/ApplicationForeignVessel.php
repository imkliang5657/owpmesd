...
class ApplicationForeignVessel
{
    ...
    public function retrieveByApplicationId(int $application_id): mixed
    {
        $query = 'SELECT AV.*, VC.vessel_category_name,
                    ...
                    FROM `application_foreign_vessel` AS AV
                    LEFT JOIN `vessel_categories` AS VC ON AV.vessel_category_id = VC.id
                    LEFT JOIN `countries` AS C1 ON AV.country_id = C1.id
                    LEFT JOIN `vessel_manufactured_information` AS VMI ON AV.manufactured_information_id = VMI.id
